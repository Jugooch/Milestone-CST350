    ...
    public class Board
    {
        ...
        public Board(int size, float difficulty)
        {
            ...
            for (int row = 0; row < Size; row++)
            {
                ...
                {
                    Grid[row, col] = new Cell(numCells, row, col);
                    numCells++;
                }
            }
        ...
        public void rightClick(int rowGuess, int colGuess)
        {
            Grid[rowGuess, colGuess].IsFlagged = !(Grid[rowGuess, colGuess].IsFlagged);
        }

        public void leftClick(int rowGuess, int colGuess)
        {
        ...
        // use recursion to clear adjacent cells that are empty.
        {
            ...
            // N.  Call flood fill on the cell north of here if it has not yet been visited.
              if (r-1 >=0  && Grid[r-1, c].IsVisited == false && Grid[r, c].LiveNeighbors == 0)
                        FloodFill(r-1, c);

            // S
            if (r + 1 < Size && Grid[r + 1, c].IsVisited == false && Grid[r, c].LiveNeighbors == 0)
                FloodFill(r + 1, c);

            // W
            if (c - 1 >= 0 && Grid[r, c-1].IsVisited == false && Grid[r, c].LiveNeighbors == 0)
                FloodFill(r, c - 1);

            // E
            if (c + 1 < Size && Grid[r, c + 1].IsVisited == false && Grid[r, c].LiveNeighbors == 0)
                FloodFill(r, c + 1);

            // NW
            if (r - 1 >= 0 && c - 1  >= 0 && Grid[r - 1, c - 1].IsVisited == false && Grid[r, c].LiveNeighbors == 0)
                FloodFill(r - 1, c - 1);

            // NE
            if (r - 1 >= 0 && c + 1 < Size && Grid[r - 1, c + 1].IsVisited == false && Grid[r, c].LiveNeighbors == 0)
                FloodFill(r - 1, c + 1);

            // SW
            if (r + 1 < Size && c - 1 >= 0 && Grid[r + 1, c - 1].IsVisited == false && Grid[r, c].LiveNeighbors == 0)
                FloodFill(r + 1, c - 1);
